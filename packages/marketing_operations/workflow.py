"""Shared LangGraph workflow for Marketing Operations Agents 22-28."""
...
from .state import MarketingOperationsState
from .tools import (
    build_assumptions,
    build_evidence,
    build_handoffs,
...
def _node_with_error_guard(node_name: str, node_fn: Callable, tel: Telemetry) -> Callable:
    def guarded(state: MarketingOperationsState) -> dict[str, Any]:
        try:
...
def _cost_ceiling(profile: AgentProfile, cfg: dict, request: MarketingOperationsRequest | None = None) -> float:
...
def _billable_llm_call(
    *,
    ...
    llm: LLMProvider,
    tel: Telemetry,
    state: MarketingOperationsState,
    messages: list[dict],
    ...
    prompt_tokens_est = estimate_prompt_tokens(messages, MarketingOperationsLLMOutput)
        ...
        with tel.span(profile.billable_stage) as span_id:
            try:
                response = llm.respond(
                    ...
                    params=params,
                    response_schema=MarketingOperationsLLMOutput,
                )
...
def _usable_llm_output(candidate: object) -> MarketingOperationsLLMOutput | None:
    if not isinstance(candidate, MarketingOperationsLLMOutput):
        return None
...
def make_intake_node(profile: AgentProfile, cfg: dict, request_cls: type[MarketingOperationsRequest], tel: Telemetry):
    _ = cfg

    def intake(state: MarketingOperationsState) -> dict[str, Any]:
...
def _safe_validation_message(exc: Exception) -> str:
    if isinstance(exc, ValidationError):
        ...
        for error in exc.errors():
            ...
            fields.append(loc)
        return "Missing or invalid Marketing Operations fields: " + ", ".join(dict.fromkeys(fields))
    return "Invalid Marketing Operations input: " + type(exc).__name__


def make_analyze_context_node(profile: AgentProfile, cfg: dict, tel: Telemetry):
    _ = cfg

    def analyze(state: MarketingOperationsState) -> dict[str, Any]:
...
def make_generate_node(profile: AgentProfile, cfg: dict, llm: LLMProvider, tel: Telemetry):
    def generate(state: MarketingOperationsState) -> dict[str, Any]:
        request = state.get("request")
...
def make_score_node(profile: AgentProfile, cfg: dict, tel: Telemetry):
    ...
    def score(state: MarketingOperationsState) -> dict[str, Any]:
...
def make_assemble_node(profile: AgentProfile, cfg: dict, package_cls: type[MarketingOperationsPackage], tel: Telemetry):
    def assemble(state: MarketingOperationsState) -> dict[str, Any]:
        request = state.get("request")
        risks = state.get("risk_flags", ())
...
def _summary(profile: AgentProfile, terminal: str, state: MarketingOperationsState) -> str:
    if terminal == "pass":
...
def _notes(profile: AgentProfile, terminal: str, state: MarketingOperationsState) -> str:
    if terminal == "stopped_cost_ceiling":
...
def build_graph(
    ...
    tel: Telemetry,
    *,
    request_cls: type[MarketingOperationsRequest] = MarketingOperationsRequest,
    package_cls: type[MarketingOperationsPackage] = MarketingOperationsPackage,
) -> Any:
    """Compile a cloud-neutral Marketing Operations LangGraph workflow."""
    ...
    def route_after_intake(state: MarketingOperationsState) -> str:
        if state.get("validation_errors"):
    ...
    def route_basic(node: str, target: str):
        def route(state: MarketingOperationsState) -> str:
    ...
    graph = StateGraph(MarketingOperationsState)
