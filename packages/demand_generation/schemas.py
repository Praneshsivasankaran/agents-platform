"""Shared typed contracts for Demand Generation Agents 08-14."""
...
class DemandGenRequest(CoreContractModel):
...
class DemandGenHandoff(CoreContractModel):
...
class DemandGenLLMOutput(CoreContractModel):
    summary: str = Field(min_length=20)
    recommendations: tuple[GeneratedRecommendation, ...] = Field(min_length=1)
    assumptions: tuple[str, ...] = ()


class DemandGenPackage(CoreContractModel):
    ...
    handoffs: tuple[DemandGenHandoff, ...] = ()
    ...
    @model_validator(mode="after")
    def _package_contract(self) -> "DemandGenPackage":
