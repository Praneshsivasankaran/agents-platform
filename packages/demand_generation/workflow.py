"""Shared LangGraph workflow for Demand Generation Agents 08-14."""
...
from .state import DemandGenState
from .tools import (
    build_assumptions,
    build_evidence,
    build_handoffs,
...
def _node_with_error_guard(node_name: str, node_fn: Callable, tel: Telemetry) -> Callable:
    def guarded(state: DemandGenState) -> dict[str, Any]:
        try:
...
def _cost_ceiling(profile: AgentProfile, cfg: dict, request: DemandGenRequest | None = None) -> float:
...
def _billable_llm_call(
    *,
    ...
    prompt_tokens_est = estimate_prompt_tokens(messages, DemandGenLLMOutput)
        ...
        with tel.span(profile.billable_stage) as span_id:
            try:
                response = llm.respond(
                    ...
                    params=params,
                    response_schema=DemandGenLLMOutput,
                )
...
def _usable_llm_output(candidate: object) -> DemandGenLLMOutput | None:
    if not isinstance(candidate, DemandGenLLMOutput):
        return None
...
def make_intake_node(profile: AgentProfile, cfg: dict, request_cls: type[DemandGenRequest], tel: Telemetry):
    _ = cfg

    def intake(state: DemandGenState) -> dict[str, Any]:
...
def _safe_validation_message(exc: Exception) -> str:
    if isinstance(exc, ValidationError):
        ...
        for error in exc.errors():
            ...
            fields.append(loc)
        return "Missing or invalid Demand Generation fields: " + ", ".join(dict.fromkeys(fields))
    return "Invalid Demand Generation input: " + type(exc).__name__


def make_analyze_context_node(profile: AgentProfile, cfg: dict, tel: Telemetry):
    _ = cfg

    def analyze(state: DemandGenState) -> dict[str, Any]:
...
def make_generate_node(profile: AgentProfile, cfg: dict, llm: LLMProvider, tel: Telemetry):
    def generate(state: DemandGenState) -> dict[str, Any]:
        request = state.get("request")
        ...
        if request is None or any(flag.severity == "hard_fail" for flag in risks):
            ...
            return {"recommendations": recommendations, "generation_used_llm": False}
        messages = [
        ...
        try:
            ...
            recommendations = build_recommendations(profile, request, evidence, risks, metric_insights)
            return {"recommendations": recommendations, "cost_usage": [stage_cost], "generation_used_llm": False}
        except CostCeilingExceeded:
            recommendations = build_recommendations(profile, request, evidence, risks, metric_insights)
            _safe_log(tel, f"{profile.billable_stage}.provider_hiccup", node=profile.billable_stage, kind="cost_ceiling")
            return {"recommendations": recommendations, "budget_limited": True, "generation_used_llm": False}
        except BillableNodeError:
            raise
        except Exception as exc:  # noqa: BLE001
            recommendations = build_recommendations(profile, request, evidence, risks, metric_insights)
...
def make_score_node(profile: AgentProfile, cfg: dict, tel: Telemetry):
    ...
    def score(state: DemandGenState) -> dict[str, Any]:
...
def make_assemble_node(profile: AgentProfile, cfg: dict, package_cls: type[DemandGenPackage], tel: Telemetry):
    def assemble(state: DemandGenState) -> dict[str, Any]:
        request = state.get("request")
        risks = state.get("risk_flags", ())
...
def _summary(profile: AgentProfile, terminal: str, state: DemandGenState) -> str:
    if terminal == "pass":
        return f"{profile.title} produced a review-ready {profile.primary_object} package."
    if terminal == "stopped_cost_ceiling":
        return f"{profile.title} returned deterministic planning guidance after the cost ceiling blocked model generation."
    if terminal == "error":
    ...
    if any(flag.severity == "hard_fail" for flag in state.get("risk_flags", ())):
        return f"{profile.title} found a hard-fail v1 scope or safety issue that requires human review."
    return f"{profile.title} produced advisory output that needs human review before use."


def _notes(profile: AgentProfile, terminal: str, state: DemandGenState) -> str:
    if terminal == "stopped_cost_ceiling":
    ...
    if any(flag.severity == "hard_fail" for flag in state.get("risk_flags", ())):
        return "Hard-fail risks require human approval and future integration design before action."
...
def build_graph(
    ...
    tel: Telemetry,
    *,
    request_cls: type[DemandGenRequest] = DemandGenRequest,
    package_cls: type[DemandGenPackage] = DemandGenPackage,
) -> Any:
    """Compile a cloud-neutral Demand Generation LangGraph workflow."""
    ...
    nodes = {
        ...
        generation_node: _node_with_error_guard(
            generation_node,
            make_generate_node(profile, cfg, llm, tel),
            tel,
        ),
    ...
    def route_after_intake(state: DemandGenState) -> str:
        if state.get("validation_errors"):
    ...
    def route_basic(node: str, target: str):
        def route(state: DemandGenState) -> str:
    ...
    graph = StateGraph(DemandGenState)
