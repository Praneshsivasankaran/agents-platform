"""Minimal internal UI for Agent 05.

FastAPI + Jinja2 + local JSON files. This is a thin wrapper around the
existing Agent 05 graph and provider factory; it does not duplicate agent logic
...
AGENT_DIR = REPO_ROOT / "agents" / "agent-05-editorial-planning"
...
from agent.schemas import CostUsage, EditorialPlanningPackage  # noqa: E402
...
app = FastAPI(title="Agent 05 Editorial Planner UI", version="0.1.0")
...
def _selected_provider_mode() -> str:
    selected = (os.environ.get("AGENT05_UI_PROVIDER") or "mock").strip().lower()
    if selected in ("mock", "offline", ""):
        return "mock"
    if selected in ("gcp", "vertex", "live"):
        return "gcp"
    raise ValueError(f"Unsupported AGENT05_UI_PROVIDER={selected!r}; expected mock or gcp")
...
def load_agent_config(mode: str | None = None) -> dict[str, Any]:
    ...
    raise ValueError("Agent 05 UI supports provider modes: mock, gcp")
...
def _package_to_dict(pkg: EditorialPlanningPackage) -> dict[str, Any]:
...
def _require_gcp_live_env() -> None:
    ...
    if missing:
        raise UserFacingError(
            "GCP live mode needs VERTEX_AI_PROJECT and Google ADC before planning. "
...
def _int_from_payload(payload: dict[str, Any], key: str, default: int) -> int:
    ...
    try:
        ...
        raise UserFacingError(f"{key} must be a number") from exc


def _build_agent_input(payload: dict[str, Any]) -> dict[str, Any]:
    start_date = _validate_required("Start date", payload.get("start_date", ""))
    end_date = _validate_required("End date", payload.get("end_date", ""))
    platforms = _split_text_items(payload.get("platforms", ""))
    pillars = _split_text_items(payload.get("content_pillars", ""))
    if not platforms:
        raise UserFacingError("Platforms are required")
    if not pillars:
        raise UserFacingError("Content pillars are required")
    return {
        "brand_name": _validate_required("Brand/company", payload.get("brand_name", "")),
        "business_goal": _validate_required("Business goal", payload.get("business_goal", "")),
        "target_audience": _validate_required("Target audience", payload.get("target_audience", "")),
        "campaign_theme": _validate_required("Campaign theme", payload.get("campaign_theme", "")),
        "platforms": platforms,
        "date_range": {"start": start_date, "end": end_date},
        "posting_frequency": {
            "cadence": str(payload.get("cadence", "weekly") or "weekly").strip().lower(),
            "count_per_week": _int_from_payload(payload, "count_per_week", 3),
            "count_per_month": _int_from_payload(payload, "count_per_month", 8),
            "total_posts": _int_from_payload(payload, "total_posts", 0),
        },
        "brand_voice": _validate_required("Tone/brand voice", payload.get("brand_voice", "")),
        "content_pillars": pillars,
        "existing_ideas": _split_text_items(payload.get("existing_ideas", "")),
        "constraints": _split_text_items(payload.get("constraints", "")),
        "priority_platforms": _split_text_items(payload.get("priority_platforms", "")),
        "excluded_topics": _split_text_items(payload.get("excluded_topics", "")),
        "key_products": _split_text_items(payload.get("key_products", "")),
        "important_dates": _split_text_items(payload.get("important_dates", "")),
        "approval_lead_time_days": _int_from_payload(payload, "approval_lead_time_days", 5),
        "production_capacity_per_week": _int_from_payload(payload, "production_capacity_per_week", 0),
        "regional_preferences": _split_text_items(payload.get("regional_preferences", "")),
    }


def run_agent(raw_input: dict[str, Any], *, provider_mode: str = "mock") -> EditorialPlanningPackage:
...
def _view_model(record: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "analysis": package.get("balance_gap_analysis") or {},
    }
