"""GCP / Vertex AI smoke pattern for Marketing Operations agents."""
...
from tests.unit.test_workflow import SAMPLE_INPUTS


_AGENT_ROOT = Path(__file__).resolve().parents[2]
_SMOKE_RUN_COST_CAP_INR = 50.0
_SAMPLE_REQUEST = SAMPLE_INPUTS[PROFILE.agent_id]
...
def test_gcp_overlay_constructs_providers_offline() -> None:
    ...
    assert PROFILE.billable_stage in cfg["llm"]["stage_tiers"]


@pytest.mark.smoke
@pytest.mark.skipif(
    not (os.environ.get("RUN_MARKETING_OPERATIONS_GCP_SMOKE") and os.environ.get("VERTEX_AI_PROJECT")),
    reason="Set RUN_MARKETING_OPERATIONS_GCP_SMOKE=1 + VERTEX_AI_PROJECT + Google ADC to run the live GCP smoke.",
)
