...
def test_request_coerces_tuple_fields() -> None:
    request = AgentRequest(
        ...
        approval_notes="Legal; Demand Gen",
        asset_list=["Landing page", "Email draft"],
    )

    assert request.channels == ("email", "paid social", "partner")
    assert request.approval_notes == ("Legal", "Demand Gen")
    assert request.asset_list == ("Landing page", "Email draft")
