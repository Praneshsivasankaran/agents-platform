...
def valid_request(**overrides: Any) -> dict[str, Any]:
    data = {
        ...
        "constraints": ["No protected-attribute targeting", "US and India"],
    }
...
def invoke(raw_input: dict[str, Any], *, cfg: dict | None = None, llm=None) -> AgentPackage:
    graph = build_graph(cfg or load_cfg(), llm or MockLLMProvider(default_scenario="pass"), StdoutTelemetry(service="agent08-test"))
...
def test_forbidden_external_action_returns_needs_human() -> None:
    package = invoke(valid_request(source_notes="Please scrape accounts and update CRM automatically."))
