...
def invoke(raw_input: dict[str, Any], *, cfg: dict | None = None, llm=None) -> AgentPackage:
    graph = build_graph(cfg or load_cfg(), llm or MockLLMProvider(default_scenario="pass"), StdoutTelemetry(service="agent19-test"))
...
def test_workflow_happy_path_returns_package() -> None:
    ...
    assert package.handoffs


def test_forbidden_external_action_returns_needs_human() -> None:
    package = invoke(valid_request(source_notes="Schedule posts and send emails automatically."))
...
def test_cost_ceiling_blocks_before_provider_call() -> None:
    ...
    cfg["cost"]["estimated_stage_cost_inr"][PROFILE.billable_stage] = 8.0
    llm = CountingLLM()
