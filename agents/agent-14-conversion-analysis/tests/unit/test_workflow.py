...
def valid_request(**overrides: Any) -> dict[str, Any]:
    data = {
        ...
        "funnel_stages": [{"count": 1000, "stage": "Visitors"}, {"count": 120, "stage": "Registrations"}, {"count": 75, "stage": "Attendees"}, {"count": 15, "stage": "Demo requests"}],
...
def invoke(raw_input: dict[str, Any], *, cfg: dict | None = None, llm=None) -> AgentPackage:
    graph = build_graph(cfg or load_cfg(), llm or MockLLMProvider(default_scenario="pass"), StdoutTelemetry(service="agent14-test"))
...
def test_workflow_happy_path_returns_package() -> None:
    ...
    assert package.metric_insights[0].value == "12.0%"


def test_forbidden_external_action_returns_needs_human() -> None:
    package = invoke(valid_request(source_notes="Pause the campaign, change budget, and launch an experiment automatically."))
