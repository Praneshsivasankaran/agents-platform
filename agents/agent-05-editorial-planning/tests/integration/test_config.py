...
def test_gcp_uses_existing_vertex_project_convention() -> None:
    ...
    assert cfg["cost"]["ceiling_inr"] == 30.0


def test_gcp_live_mode_uses_predictable_cost_controls() -> None:
    cfg = yaml.safe_load((ROOT / "config" / "gcp.yaml").read_text(encoding="utf-8"))

    assert cfg["llm"]["provider_call_max_attempts"] == 1
    assert set(cfg["llm"]["stage_tiers"].values()) == {"cheap"}
    assert cfg["llm"]["max_full_briefs_live_mode"] == 5
    assert cfg["cost"]["max_output_tokens"]["map_platform_strategy"] >= 4096
    assert cfg["cost"]["max_output_tokens"]["generate_content_briefs"] >= 8192
