"""Export Agent 04 Markdown docs to simple review PDFs.

This script intentionally keeps formatting conservative so the PDFs are
repeatable in local/offline environments. It uses ReportLab directly instead
of depending on a system Markdown or browser renderer.
"""
...
def _register_font() -> tuple[str, str]:
    """Return regular/bold font names, preferring Unicode-capable fonts."""
    ...
    for regular_path, bold_path in zip(candidates, bold_candidates, strict=False):
        if regular_path.exists() and bold_path.exists():
            pdfmetrics.registerFont(TTFont("Agent04Regular", str(regular_path)))
            pdfmetrics.registerFont(TTFont("Agent04Bold", str(bold_path)))
            return "Agent04Regular", "Agent04Bold"
...
def _footer(canvas, doc) -> None:
    ...
    canvas.drawString(0.72 * inch, 0.45 * inch, "Agent 04 SEO Optimizer")
