...
def draft_prompt(
    ...
    return (
        ...
        "not reproduce the source verbatim. Return the raw blog Markdown only; do not wrap "
        "the post in ```markdown, code fences, XML, JSON, or commentary. Write like a human "
        "blog editor talking to the intended reader: clear, warm, direct, and practical. "
        "Open with a relatable hook grounded in the source material, not a generic phrase. "
        "Include all planned sections with appropriate headings, a compelling introduction, "
        "natural transitions, and a clear conclusion. Vary sentence length and paragraph "
        "shape so the piece feels edited, not templated. Prefer specific, plain-language examples, "
        "concrete trade-offs, and practical next steps over broad claims. Use bullets only when they help the "
        "reader scan; do not turn every section into a list. Avoid generic marketing phrases, "
        "empty hype, and overused lines such as 'poised to become', 'game changer', "
        "'unlock new levels', 'in today's fast-paced world', or 'remarkable opportunity'. "
        "Make every section useful to the intended reader."
        + revision_section
...
def review_prompt(
    ...
    *,
    ...
    return (
        ...
        "  engagement_value (0-5): human voice, hook, CTA, reader engagement\n\n"
        "Penalize drafts that read like generic AI filler, a sterile outline, or a template. "
        "Penalize drafts wrapped in code fences such as ```markdown; the final draft should "
        "be raw blog Markdown, not a displayed code block.\n\n"
